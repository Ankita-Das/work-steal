...
We have attempted an implementation of where deques are alloted from a shared
buffer pool as described in section 4 of \cite{chase2005dynamic}. Some initial outputs
from comparing memory manangement after deque shrinking/growing via the OCaml GC vs
using a shared buffer memory pool for manually managing deques shrinks/growths
are here:
...
The outputs show <> . This work has also brought up questions such as the <>
We stress that this implementation requires refinement and that we
would like to continue working on this in the near future. Comparing such an
implementation against an equivalent scheduler in Rust using
\href{https://docs.rs/crossbeam-deque/latest/crossbeam_deque/}{\texttt{crossbeam}}
or our own deque implementation — would help us understand
acquire-release semantics much better as well as the practical differences between
...
\midrule
Alina Banerjee  & 33 \% & deque, circular array, scheduler implementation,
                          qcheck tests\\
